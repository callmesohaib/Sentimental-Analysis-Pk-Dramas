...
def extract_episode_number(dramaname_ep):
    # Handle both filename patterns: "Ehsaan FaramoshEp1" and "Ehsaan FaramoshEp1_Urdu_T"
...
def parse_subtitle_file(filepath):
    """Parse subtitle file and return dict of {timestamp: text}"""
    subtitles = {}
    try:
        with open(filepath, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if line:  # Skip empty lines
                    # Handle both formats: [timestamp] text and plain text
                    if line.startswith('['):
                        timestamp, text = re.match(
                            r'^\[(.*?)\]\s*(.*)', line).groups()
                        subtitles[timestamp] = text
                    else:
                        # Use "-" as timestamp for lines without one
                        subtitles["-"] = line
    except Exception as e:
...
def generate_final_csv(base_path, drama_playlist_mapping):
    output_csv = os.path.join(base_path, "Dataset.csv")
    ...
    with codecs.open(output_csv, mode='w', encoding='utf-8-sig') as file:
        ...
        for drama_name, info in drama_playlist_mapping.items():
            ...
            for urdu_filename in os.listdir(urdu_folder):
                ...
                english_filepath = os.path.join(
                    english_folder, english_filename)

                if not os.path.exists(english_filepath):
                    print(
                        f"⚠️ Matching English file not found: {english_filename}")
                    continue

                # Parse both subtitle files
                urdu_subtitles = parse_subtitle_file(urdu_filepath)
                english_subtitles = parse_subtitle_file(english_filepath)

                if not urdu_subtitles:
                    print(
                        f"⚠️ No subtitles found in Urdu file: {urdu_filename}")
                    continue
                if not english_subtitles:
                    print(
                        f"⚠️ No subtitles found in English file: {english_filename}")
                    continue

                # Get episode info
                episode_name = urdu_filename.split('_')[0].replace('.txt', '')
                episode_number = extract_episode_number(episode_name)
                episode_url = playlist_videos[episode_number -
                                              1] if episode_number and episode_number <= len(playlist_videos) else "N/A"

                # Find common timestamps between Urdu and English
                common_timestamps = set(urdu_subtitles.keys()) & set(
                    english_subtitles.keys())
                if not common_timestamps:
                    print(
                        f"⚠️ No matching timestamps between {urdu_filename} and {english_filename}")
                    continue

                # Write matched subtitles to CSV
                sentence_no = 1
                for timestamp in sorted(common_timestamps):
                    writer.writerow([
                        ...
                        timestamp,
                        sentence_no,
                        urdu_subtitles[timestamp],
                        english_subtitles[timestamp],
                        "",  # Sentiment column
                    ])
                    sentence_no += 1

                print(
                    f"✅ Processed {sentence_no-1} subtitle pairs from {urdu_filename}")
...
generate_final_csv(base_path, drama_playlist_mapping)
